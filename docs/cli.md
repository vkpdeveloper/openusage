...
Failed provider refreshes do not overwrite the last successful cached snapshot. CLI output only includes cached snapshots fetched in the last 5 minutes.
...
Only snapshots fetched in the last 5 minutes are printed.
...
Providers with multiple saved profiles return one JSON object per profile.
